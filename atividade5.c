...
void reorganizarEntidades(Entidade *jogador) {
    int menorIndice = 1;

    float dx = jogador->pos.x - vetorEntidades[1]->pos.x;
    float dy = jogador->pos.y - vetorEntidades[1]->pos.y;
    float menorDist = dx * dx + dy * dy;

    for (int i = 2; i < totalEntidades; i++) {
        dx = jogador->pos.x - vetorEntidades[i]->pos.x;
        dy = jogador->pos.y - vetorEntidades[i]->pos.y;
    ...
    if (menorIndice != 1) {
        ...
        vetorEntidades[1] = vetorEntidades[menorIndice];
        vetorEntidades[menorIndice] = tmp;
    }
}   
